...
var uploadTools = {
    ...
    "getInitOption":function(uploadId){
        ...
        var initOption={
            "uploadId":uploadId,
            "uploadUrl":"http://139.196.120.14/uploadfile.do",//必须，上传地址
...
//              console.info(isImgUrl+"图片啊")
    ...
    "initWithLayout":function(opt){
        ...
        var btsStr = "";
	        ...
	        btsStr += "<div>";
	        btsStr += "<div class='selectFileBt' style='display:inline-block'>选取点赞图片</div>";
	       	btsStr += "</div>";
...
//      console.info(opt.uploadUrl);
		
        var fileList = uploadFileList.getFileList(opt);
		var rememberFile = [];
		
        var formData = new FormData($( "#fileUploadContent" )[0]);
        var fileNumber = uploadTools.getFileNumber(opt);
...
//      }
       ...
       for(var i=0;i<fileList.length;i++){
            if(fileList[i]!=null){
                ...
                console.info(formData+"form11111")
            }
        }
        if(opt.otherData!=null&&opt.otherData!=""){
            for(var j=0;j<opt.otherData.length;j++){
                ...
                console.info(formData+"form22222")
            }
        }

        formData.append("filelSavePath",opt.filelSavePath);
        console.info(formData+"form333333")
         //if(fileNumber==2){//????
         if(true){	
       	  //uploadTools.showDailog();       //显示弹窗
...
//					console.info(JSON.stringify(formData));
					...
					$.ajax({
		                ...
		                success:function(data){
		                    ...
		                    uploadEvent.cleanFileEvent(opt);
		                    console.info("ajax上传了");
		                },
		                error:function(e){
							console.info("ajax错误啦")
		                }
		            });
...
//  	console.info("哈哈哈"+opt.fileList+"拉阿拉")
        return opt.fileList;
    },
...
var formTake={
    "getData":function(formId){
        ...
        console.info(input_doms+select_doms+"哒哒哒")
